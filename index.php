...
if($_GET[logout]==1) { logout(); header("Location: ".PREFIX); }
if(!isset($_SESSION[id])) { $_SESSION[id] = 0; } // Set id to 0 if we aren't logged in

if(!isset($_GET[category])) { $_GET[category] = NULL; }
elseif(!categoryExists($_GET[category])) { header("Location: ".PREFIX); } // Redirect back to start if category doesn't exist

if(!isset($_GET[domain])) { $_GET[domain] = NULL; }
elseif(!domainExists($_GET[domain])) { header("Location: ".PREFIX); } // Redirect if the domain doesn't exist
